  ...
  async function handleMovieDownloadWithModal(movie, loadingMessage) {
    ...
    try {
        ...
        await handleMovieDownload(
            ...
            error => createErrorDialog('Error during download process', () => handleMovieDownloadWithModal(movie, loadingMessage))
        );
        ...
        createErrorDialog(error.message || 'Unknown error', () => handleMovieDownloadWithModal(movie, loadingMessage));
    } finally {
        setTimeout(() => modalStack.closeTopmost(), 1000);
    }
}
...
<DetachedPage>
  <div class="relative">
          ...
          {#await Promise.all([$jellyfinItemP, $radarrItemP]) then [jellyfinItem, radarrItem]}
            <Container direction="horizontal" class="flex mt-8">
              ...
              {#if !$requestExists && !jellyfinItem}
                <Button class="mr-4" on:clickOrSelect={handleRequest} disabled={!allowRequests}>
                  Request
    ...
    {#await Promise.all([tmdbMovie, radarrFiles, radarrDownloads]) then [movie, files, downloads]}
      {#if currentUser?.isAdmin && files?.length || downloads?.length}
