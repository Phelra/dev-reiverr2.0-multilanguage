...
async getViewId(type: Type): Promise<string | undefined> {
    if (this.views[type]) return this.views[type]?.Id;

    const views = await this.getClient()
    ...
    this.views[type] = views.find((view) =>
        type === 'movie'
...
getRecentlyAdded = async (type: Type = 'series'): Promise<JellyfinItem[]> => {
    return this.getClient()
        .GET('/Users/{userId}/Items', {
            params: {
                path: {
                ...
                query: {
                    ...
                    recursive: true,
                    fields: ['ProviderIds', 'Genres', 'DateCreated']
                }
            }
        })
        .then((r) => r.data?.Items || []);
...
async getVirtualFolders(): Promise<VirtualFolderInfo[]> {
    ...
    async getLibraryItemsByLibraryId(libraryId: string): Promise<JellyfinItem[]> {
        return this.getClient()
            .GET('/Users/{userId}/Items', {
                params: {
                    path: {
            ...
            .then((response) => response.data?.Items || [])
            .catch((error) => {
                console.error(`Error fetching items for library ${libraryId}:`, error);
                return [];
            });
    }

    async getSubfoldersByLibraryId(libraryId: string): Promise<JellyfinItem[]> {
        return this.getClient()
            .GET('/Users/{userId}/Items', {
                params: {
                    path: {
            ...
            .then((response) => response.data?.Items || [])
            .catch((error) => {
                console.error(`Error fetching subfolders for library ${libraryId}:`, error);
                return [];
            });
    }
